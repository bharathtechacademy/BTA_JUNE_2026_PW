...
Go to the terminal (Cntrl+`) => cd C:\Training\PlaywrightTrainings\June_2026\playwright-bdd-framework

## 2. Install all the necessary dependencies based on our project requirements. 
1. **TypeScript** — `npm install typescript ts-node @types/node` and `npx tsc --init` for language support.
2. **Playwright** - `npm init playwright@latest` (or add `@playwright/test`) for web / API automation with the Playwright Test runner.
...
6. **Cucumber** - `npm install @cucumber/cucumber` To set up a Cucumber BDD framework and to integrate a feature file along with step definitions 
