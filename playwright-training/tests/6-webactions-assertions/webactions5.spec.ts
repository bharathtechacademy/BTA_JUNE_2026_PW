...
test('Assignment - Demo QA', async ({ page }) => {
    ...
    const logo = await page.locator('img[src*="Toolsqa"]');
    await logo.screenshot({ path: 'files/screenshot1.png' });
    await page.screenshot({ path: 'files/screenshot2.png' })
    await page.screenshot({ path: 'files/screenshot3.png', fullPage: true })
    ...
    // 12. close the popup
    const closeButton = await page.locator('button#closeLargeModal');
