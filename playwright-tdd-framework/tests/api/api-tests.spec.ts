...
test.describe('GIT API Tests', () => {
    ...
    //Test Case 3: Request to get a valid repository within GitHub. 
    test('Get a valid repository', async () => {
    ...
    //Test Case 4: Request to update a valid repository within GitHub. 
    test('Update a valid repository', async () => {
        const data = testdata.updateRepo;
        await api.getResponse(data.requestType, data.endpoint, data.payload);
        await api.validateStatusCode(data.expCode);
        await api.validateStatusMessage(data.expMessage);
        await api.validateResponseBody("description", data.description);
        await api.validateResponseSchema("description", data.expDataType);
    });

    //Test Case 5: Request to delete a valid repository within GitHub. 
    test('Delete a valid repository', async () => {
